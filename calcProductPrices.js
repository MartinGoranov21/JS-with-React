...
function calcTotalPrice(products) {
	// YOUR CODE HERE
   const TotalPrice= products.map(el=> el.price).reduce((acc, curr)=>acc+curr);
   return TotalPrice;
}
...
function calcTotalPriceByCategory(products, category) {
	// YOUR CODE HERE
    const PriceByCategory = products.filter(el=> el.category=== category).map(el=> el.price).reduce((acc, curr)=> acc+curr);
